...
import {
  DaoGovernanceDtoQuery,
  DaoGovernanceQuery,
} from "../../../../types/proposals/daoGovernance-types";
import { crudTestCreator } from "../crudTestCreator";

export const daoGovernanceRepositoryTests = (repositories: any) => {
  const { daoGovernanceRepository } = repositories;

  const daoGovernance1: DaoGovernanceDtoQuery = new DaoGovernance({
    proposal: {
      name: "Create SubDAO for Community Growth",
      description:
        "Proposal to create a new SubDAO focused on community-driven initiatives and engagement.",
      relatedLinks: [
  ...
  const daoGovernance2: DaoGovernanceDtoQuery = new DaoGovernance({
    proposal: {
      ...
      description:
        "Proposal to reallocate 50,000 USD from the general reserve to a strategic initiatives fund.",
  ...
  crudTestCreator<DaoGovernanceDtoQuery, DaoGovernanceQuery>(

...
const AppNav = async () => {
  ...
  return (
    <nav className="p-2 flex flex-row gap-2 items-center">
      {user && (
        <>
          {user.nickname !== null && <Button asChild className="text-white border border-white/5 bg-transparent hover:bg-[#0B1739] p-6 rounded-[222px]">
      ...
      {!user && (
        <>
          <Button className="text-white border border-[#7912FF] bg-transparent hover:bg-[#0B1739] p-6 rounded-[24px]">
            <Link href="login/" className="flex"><Image src="/images/wallet.png" alt="" width={16} height={16}  className="mr-2"/>Connect wallet</Link>
          </Button>
        </>
      )}
    </nav>
